...
class Discountcoffee_Quickorder_Model_Cart extends Varien_Object implements Mage_Checkout_Model_Cart_Interface
{
    /**
    ...
    {
        if (!$orderItem->getId()) {
            Mage::throwException(Mage::helper('discountcoffee_quickorder')->__('Failed to reorder item'));
        }
        ...
        if ($product->getId()) {
            ...
            Mage::throwException(Mage::helper('discountcoffee_quickorder')->__('Failed to add a product of order item'));
        }
    }
    ...
    public function prepareAddProductsBySku(array $items)
    {
        foreach ($items as $item) {
            ...
            if ($item['code'] != Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_EMPTY) {
     ...
     * [
     *  'status' => string (see Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_* constants),
    ...
    public function getQtyStatus(
        ...
        if ($result->getHasError()) {
            ...
            switch ($result->getErrorCode()) {
                case 'qty_increments':
                    $status = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_QTY_INCREMENTS;
                    $return['qty_increments'] = $stockItem->getQtyIncrements();
                    break;
                case 'qty_min':
                    $status = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_QTY_ALLOWED_IN_CART;
                ...
                case 'qty_max':
                    ...
                    break;
                default:
                    $status = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_QTY_ALLOWED;
        ...
        if ($product && $product->getId()) {
            ...
            if ($this->_isCheckout() && $product->isDisabled()) {
                ...
                $failCode = $this->_context == self::CONTEXT_FRONTEND
                    ? Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_SKU
                    : Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_DISABLED;
                return $this->_updateItem($item, $failCode);
            }

            if ($this->_isFrontend() && true === $product->getDisableAddToCart()) {
                return $this->_updateItem($item, Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_PERMISSIONS);
            }
            ...
            if ($this->_isCheckout() && $this->_isProductOutOfStock($product)) {
                $item['is_configure_disabled'] = true;
                return $this->_updateItem($item, Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_OUT_OF_STOCK);
            }
            ...
            if ($this->_shouldBeConfigured($product)) {
                if (!$this->_isConfigured($product, $config)) {
                    $failCode = (!$this->_isFrontend() || $product->isVisibleInSiteVisibility())
                        ? Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_CONFIGURE
                        : Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_SKU;
                    return $this->_updateItem($item, $failCode);
                } else {
                    $item['code'] = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_SUCCESS;
                }
            }

            if ($prevalidateStatus != Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_SUCCESS) {
            ...
            if ($this->_isFrontend() && !$item['is_qty_disabled']) {
                ...
                if ($qtyStatus === true) {
                    return $this->_updateItem($item, Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_SUCCESS);
                } else {
            ...
            return $this->_updateItem($item, Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_SKU);
        }
    ...
    protected function _validateProductWebsite($product)
    {
        ...
        return (Mage::app()->getStore()->isAdmin())
            ? Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_WEBSITE
            : Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_SKU;
    }
    ...
    {
        $code = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_SUCCESS;
        if ($sku == '') {
            $code = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_EMPTY;
        } else {
            if (!Zend_Validate::is($qty, 'Float')) {
                $code = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_QTY_INVALID_NUMBER;
            } else {
                $qty = Mage::app()->getLocale()->getNumber($qty);
                if ($qty <= 0) {
                    $code = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_QTY_INVALID_NON_POSITIVE;
                } elseif ($qty < 0.0001 || $qty > 99999999.9999) {
                    // same as app/design/frontend/enterprise/default/template/checkout/widget/sku.phtml
                    $code = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_QTY_INVALID_RANGE;
                }
            }
        }

        if ($code != Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_SUCCESS) {
            $qty = '';
        }
    ...
    public function saveAffectedProducts(Mage_Checkout_Model_Cart_Interface $cart = null, $saveQuote = true)
    {
        ...
        foreach ($affectedItems as &$item) {
            if ($item['code'] == Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_SUCCESS) {
    ...
    protected function _safeAddProduct(&$item, Mage_Checkout_Model_Cart_Interface $cart, $suppressSuperMode = false)
    {
        ...
        try {
            ...
            if (!$suppressSuperMode) {
                $success = false;
                $item['code'] = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_UNKNOWN;
                if ($this->_isFrontend()) {
            ...
            $success = false;
            $item['code'] = Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_UNKNOWN;
            $error = Mage::helper('discountcoffee_quickorder')->__('The product cannot be added to cart.');
            if ($this->_isFrontend()) {
     ...
     *  ],
     *  'code' => string (see Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_*)
     * ]
     *
    ...
    public function getMessages()
    {
        ...
        foreach ($this->_currentlyAffectedItems as $sku) {
            if (isset($affectedItems[$sku])
                && $affectedItems[$sku]['code'] != Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_SUCCESS
            ) {
        ...
        if ($addedItemsCount) {
            $message = ($addedItemsCount == 1)
                    ? Mage::helper('discountcoffee_quickorder')->__('%s product was added to your shopping cart.', $addedItemsCount)
                    : Mage::helper('discountcoffee_quickorder')->__('%s products were added to your shopping cart.', $addedItemsCount);
            $messages[] = Mage::getSingleton('core/message')->success($message);
        }
        if ($failedItemsCount) {
            $warning = ($failedItemsCount == 1)
                    ? Mage::helper('discountcoffee_quickorder')->__('%s product requires your attention.', $failedItemsCount)
                    : Mage::helper('discountcoffee_quickorder')->__('%s products require your attention.', $failedItemsCount);
    ...
    {
        if (!isset($item['sku']) || $code == Discountcoffee_Quickorder_Helper_Data::ADD_ITEM_STATUS_FAILED_EMPTY) {
            return $this;
        }
        $sku = $item['sku'];
        $affectedItems = $this->getAffectedItems();
     ...
     *
     * @return Discountcoffee_Quickorder_Helper_Data
     */
    protected function _getHelper()
    {
        return Mage::helper('discountcoffee_quickorder');
    }
